...
class QueueIntegrationMixin:
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                response = await client.post(
                    f"{self.queue_url}/jobs/submit", json=job_request, timeout=30.0
                )
    ...
    async def _prepare_job_parameters(
    ...
    async def get_job_status(self, job_id: str) -> Dict[str, Any]:
        ...
        async with httpx.AsyncClient() as client:
            response = await client.get(
                f"{self.queue_url}/jobs/{job_id}/status", timeout=10.0
            )
    ...
    async def get_job_result(self, job_id: str) -> Dict[str, Any]:
        ...
        async with httpx.AsyncClient() as client:
            response = await client.get(
                f"{self.queue_url}/jobs/{job_id}/result", timeout=30.0
            )
    ...
    async def cancel_job(self, job_id: str) -> Dict[str, Any]:
        ...
        async with httpx.AsyncClient() as client:
            response = await client.delete(
                f"{self.queue_url}/jobs/{job_id}", timeout=10.0
            )
